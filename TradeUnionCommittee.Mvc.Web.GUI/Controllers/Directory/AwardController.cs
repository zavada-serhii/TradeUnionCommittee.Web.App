...
namespace TradeUnionCommittee.Mvc.Web.GUI.Controllers.Directory
{
    public class AwardController : Controller
    {
        ...
        public AwardController(IAwardService services, IOops oops, IMapper mapper, ISystemAuditService systemAuditService)
        {
            _services = services;
            _oops = oops;
            _mapper = mapper;
        ...
        public async Task<IActionResult> Create(AwardViewModel vm)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.IsValid)
                {
                    await _systemAuditService.AuditAsync(User.Identity.Name, Operations.Insert, Tables.Award);
        ...
        public async Task<IActionResult> Update(string id)
        {
            ...
            return result.IsValid ? View(_mapper.Map<AwardViewModel>(result.Result)) : _oops.OutPutError("Award", "Index", result.ErrorsList);
        }
        ...
        public async Task<IActionResult> UpdateConfirmed(AwardViewModel vm)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.IsValid)
                {
                    await _systemAuditService.AuditAsync(User.Identity.Name, Operations.Update, Tables.Award);
                    return RedirectToAction("Index");
                }
                return _oops.OutPutError("Award", "Index", result.ErrorsList);
            }
        ...
        public async Task<IActionResult> DeleteConfirmed(string id)
        {
            ...
            if (result.IsValid)
            {
                await _systemAuditService.AuditAsync(User.Identity.Name, Operations.Delete, Tables.Award);
                return RedirectToAction("Index");
            }
            return _oops.OutPutError("Award", "Index", result.ErrorsList);
        }
