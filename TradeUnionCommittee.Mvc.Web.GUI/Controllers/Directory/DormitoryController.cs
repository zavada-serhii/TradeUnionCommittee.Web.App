...
namespace TradeUnionCommittee.Mvc.Web.GUI.Controllers.Directory
{
    public class DormitoryController : Controller
    {
        ...
        public DormitoryController(IDormitoryService services, IMapper mapper, ISystemAuditService systemAuditService, IHttpContextAccessor accessor)
        {
            _services = services;
            _mapper = mapper;
        ...
        public async Task<IActionResult> Update([Required] string id)
        {
            ...
            if (result.IsValid)
            {
                return View(_mapper.Map<UpdateDormitoryViewModel>(result.Result));
            }
            TempData["ErrorsList"] = result.ErrorsList;
            return View();
        }
