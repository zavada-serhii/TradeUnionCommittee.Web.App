...
{
    public class TourEmployeesController : Controller
    {
        private readonly ITourEmployeesService _services;
        private readonly IDirectories _directories;
        private readonly IMapper _mapper;
        private readonly ISystemAuditService _systemAuditService;
        private readonly IHttpContextAccessor _accessor;

        public TourEmployeesController(ITourEmployeesService services, IDirectories directories, IMapper mapper, ISystemAuditService systemAuditService, IHttpContextAccessor accessor)
        {
            _services = services;
            _mapper = mapper;
        ...
        public async Task<IActionResult> Create(CreateEventEmployeesViewModel vm)
        {
            ...
            {
                var result = await _services.CreateAsync(_mapper.Map<TourEmployeesDTO>(vm));
                if (result.IsValid)
                {
        ...
        public async Task<IActionResult> Update(UpdateEventEmployeesViewModel vm)
        {
            if (ModelState.IsValid)
            {
                var result = await _services.UpdateAsync(_mapper.Map<TourEmployeesDTO>(vm));
                if (result.IsValid)
                {
