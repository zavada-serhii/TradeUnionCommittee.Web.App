...
namespace TradeUnionCommittee.Razor.Web.GUI.Controllers.Directory
{
    public class MaterialAidController : Controller
    {
        ...
        public MaterialAidController(IMaterialAidService services, IMapper mapper, ISystemAuditService systemAuditService, IHttpContextAccessor accessor)
        {
            _services = services;
            _mapper = mapper;
        ...
        public async Task<IActionResult> Update([Required] string id)
        {
            ...
            if (result.IsValid)
            {
                return View(_mapper.Map<UpdateMaterialAidViewModel>(result.Result));
            }
            TempData["ErrorsList"] = result.ErrorsList;
            return View();
        }
