...
{
    public class TravelFamilyController : Controller
    {
        private readonly ITravelFamilyService _services;
        ...
        public TravelFamilyController(ITravelFamilyService services, IDirectories directories, IMapper mapper, ISystemAuditService systemAuditService, IHttpContextAccessor accessor, IReferenceParentService referenceParent)
        {
            _services = services;
            _mapper = mapper;
        ...
        public async Task<IActionResult> Create(CreateEventFamilyViewModel vm)
        {
            ...
            {
                var result = await _services.CreateAsync(_mapper.Map<TravelFamilyDTO>(vm));
                if (result.IsValid)
                {
        ...
        public async Task<IActionResult> Update(UpdateEventFamilyViewModel vm)
        {
            if (ModelState.IsValid)
            {
                var result = await _services.UpdateAsync(_mapper.Map<TravelFamilyDTO>(vm));
                if (result.IsValid)
                {
