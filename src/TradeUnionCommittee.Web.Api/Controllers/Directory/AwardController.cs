...
namespace TradeUnionCommittee.Web.Api.Controllers.Directory
{
    ...
    public class AwardController : ControllerBase
    {
        ...
        public AwardController(IAwardService services, ISystemAuditService systemAuditService, IMapper mapper, IHttpContextAccessor accessor, ILogger<AwardController> logger)
        {
            _services = services;
        ...
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        [ProducesResponseType(typeof(CreateAwardViewModel), StatusCodes.Status201Created)]
        ...
        public async Task<IActionResult> Create([FromBody] CreateAwardViewModel vm)
        {
            ...
            if (result.IsValid)
            {
                ...
                return CreatedAtRoute("GetAward", new { id = result.Result }, vm);
            }
        ...
        public async Task<IActionResult> Update([FromBody] UpdateAwardViewModel vm)
        {
            ...
            if (result.IsValid)
            {
                await _systemAuditService.AuditAsync(User.GetEmail(), _accessor.GetIp(), Operations.Update, Tables.Award);
                return NoContent();
            }
            return BadRequest(result.ErrorsList);
        }
        ...
        public async Task<IActionResult> Delete([Required] string id)
        {
            ...
            if (result.IsValid)
            {
                await _systemAuditService.AuditAsync(User.GetEmail(), _accessor.GetIp(), Operations.Delete, Tables.Award);
                return NoContent();
            }
            return NotFound(result.ErrorsList);
        }
