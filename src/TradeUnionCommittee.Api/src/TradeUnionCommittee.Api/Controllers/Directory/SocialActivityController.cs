...
namespace TradeUnionCommittee.Api.Controllers.Directory
{
    ...
    public class SocialActivityController : ControllerBase
    {
        ...
        private readonly ILogger<SocialActivityController> _logger;

        public SocialActivityController(ISocialActivityService services, ISystemAuditService systemAuditService, IMapper mapper, IHttpContextAccessor accessor, ILogger<SocialActivityController> logger)
        {
            _services = services;
        ...
        [HttpGet]
        [Route("Get/{id}", Name = "GetSocialActivity")]
        [MapToApiVersion("1.0")]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        ...
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        [ProducesResponseType(typeof(CreateSocialActivityViewModel), StatusCodes.Status201Created)]
        ...
        public async Task<IActionResult> Create([FromBody] CreateSocialActivityViewModel vm)
        {
            ...
            if (result.IsValid)
            {
                await _systemAuditService.AuditAsync(User.GetEmail(), _accessor.GetIp(), Operations.Insert, Tables.SocialActivity);
                return CreatedAtRoute("GetSocialActivity", new { version = "1.0", controller = "SocialActivity", id = result.Result }, vm);
            }
        ...
        public async Task<IActionResult> Update([FromBody] UpdateSocialActivityViewModel vm)
        {
            ...
            if (result.IsValid)
            {
                await _systemAuditService.AuditAsync(User.GetEmail(), _accessor.GetIp(), Operations.Update, Tables.SocialActivity);
                return NoContent();
            }
            return BadRequest(result.ErrorsList);
        }
        ...
        public async Task<IActionResult> Delete([Required] string id)
        {
            ...
            if (result.IsValid)
            {
                await _systemAuditService.AuditAsync(User.GetEmail(), _accessor.GetIp(), Operations.Delete, Tables.SocialActivity);
                return NoContent();
            }
            return NotFound(result.ErrorsList);
        }
