...
{
    //ITravelGrandChildrenService
    //TravelGrandChildrenDTO
    //CreateEventGrandChildrenViewModel
    //UpdateEventGrandChildrenViewModel

    public class TravelGrandChildrenController : Controller
    {
        private readonly ITravelGrandChildrenService _services;
        ...
        public TravelGrandChildrenController(ITravelGrandChildrenService services, IDirectories directories, IMapper mapper, ISystemAuditService systemAuditService, IHttpContextAccessor accessor, IReferenceParent referenceParent)
        {
            _services = services;
            _mapper = mapper;
        ...
        public async Task<IActionResult> Create(CreateEventGrandChildrenViewModel vm)
        {
            ...
            {
                var result = await _services.CreateAsync(_mapper.Map<TravelGrandChildrenDTO>(vm));
                if (result.IsValid)
                {
        ...
        public async Task<IActionResult> Update(UpdateEventGrandChildrenViewModel vm)
        {
            if (ModelState.IsValid)
            {
                var result = await _services.UpdateAsync(_mapper.Map<TravelGrandChildrenDTO>(vm));
                if (result.IsValid)
                {
