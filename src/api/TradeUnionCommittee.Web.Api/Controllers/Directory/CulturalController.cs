...
namespace TradeUnionCommittee.Web.Api.Controllers.Directory
{
    ...
    public class CulturalController : ControllerBase
    {
        ...
        private readonly ILogger<CulturalController> _logger;

        public CulturalController(ICulturalService services, ISystemAuditService systemAuditService, IMapper mapper, IHttpContextAccessor accessor, ILogger<CulturalController> logger)
        {
            _services = services;
        ...
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        [ProducesResponseType(typeof(CreateCulturalViewModel), StatusCodes.Status201Created)]
        ...
        public async Task<IActionResult> Update([FromBody] UpdateCulturalViewModel vm)
        {
            ...
            if (result.IsValid)
            {
                await _systemAuditService.AuditAsync(User.GetEmail(), _accessor.GetIp(), Operations.Update, Tables.Cultural);
                return NoContent();
            }
            return BadRequest(result.ErrorsList);
        }
        ...
        public async Task<IActionResult> Delete([Required] string id)
        {
            ...
            if (result.IsValid)
            {
                await _systemAuditService.AuditAsync(User.GetEmail(), _accessor.GetIp(), Operations.Delete, Tables.Cultural);
                return NoContent();
            }
            return NotFound(result.ErrorsList);
        }
