...
namespace TradeUnionCommittee.BLL.Services.Directory
{
    public class SocialActivityService : ISocialActivityService
    {
        ...
        public SocialActivityService(IUnitOfWork database, IAutoMapperUtilities mapperService, IHashIdUtilities hashIdUtilities)
        {
            _database = database;
        ...
        {
            var check = await _hashIdUtilities.CheckDecryptWithId(hashId, Enums.Services.SocialActivity);
            return check.IsValid
                ? _mapperService.Mapper.Map<ActualResult<DirectoryDTO>>(await _database.SocialActivityRepository.Get(check.Result))
        ...
        {
            var check = await _hashIdUtilities.CheckDecryptWithId(dto.HashId, Enums.Services.SocialActivity);
            if (check.IsValid)
            {
                if (!await CheckNameAsync(dto.Name))
                {
        ...
        public async Task<ActualResult> DeleteAsync(string hashId)
        {
            ...
            if (check.IsValid)
            {
                await _database.SocialActivityRepository.Delete(check.Result);
                return _mapperService.Mapper.Map<ActualResult>(await _database.SaveAsync());
            }
            return new ActualResult(check.ErrorsList);
        }
