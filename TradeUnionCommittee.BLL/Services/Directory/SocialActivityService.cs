...
namespace TradeUnionCommittee.BLL.Services.Directory
{
    public class SocialActivityService : ISocialActivityService
    {
        ...
        public SocialActivityService(IUnitOfWork database, IAutoMapperUtilities mapperService, ICheckerService checkerService)
        {
            _database = database;
        ...
        {
            var check = await _checkerService.CheckDecryptWithId(hashId, Enums.Services.SocialActivity);
        ...
        {
            var check = await _checkerService.CheckDecryptWithId(dto.HashId, Enums.Services.SocialActivity);
            if (check.IsValid)
            {
                if (!await CheckNameAsync(dto.Name))
                {
