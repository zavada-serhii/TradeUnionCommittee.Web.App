...
namespace TradeUnionCommittee.BLL.Services.Directory
{
    public class PositionService : IPositionService
    {
        ...
        {
            var check = await _checkerService.CheckDecryptWithId(hashId, Enums.Services.Position);
        ...
        {
            var check = await _checkerService.CheckDecryptWithId(dto.HashId, Enums.Services.Position);
            if (check.IsValid)
            {
                if (!await CheckNameAsync(dto.Name))
                {
