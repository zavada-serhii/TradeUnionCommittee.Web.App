...
namespace TradeUnionCommittee.BLL.Services.Directory
{
    public class CulturalService : ICulturalService
    {
        ...
        public CulturalService(IUnitOfWork database, IAutoMapperUtilities mapperService, IHashIdUtilities hashIdUtilities)
        {
            _database = database;
        ...
        {
            var check = await _hashIdUtilities.CheckDecryptWithId(hashId, Enums.Services.Cultural);
        ...
        {
            var check = await _hashIdUtilities.CheckDecryptWithId(dto.HashId, Enums.Services.Cultural);
            if (check.IsValid)
            {
                if (!await CheckNameAsync(dto.Name))
                {
