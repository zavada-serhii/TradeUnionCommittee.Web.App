...
namespace TradeUnionCommittee.Web.GUI.Controllers.Directory
{
    public class CulturalController : Controller
    {
        ...
        public CulturalController(ICulturalService services, IOops oops)
        {
            _services = services;
            _oops = oops;
        }
        ...
        public async Task<IActionResult> Create([Bind("Name")] CulturalViewModel vm)
        {
        ...
        public async Task<IActionResult> Update(long? id)
        {
            ...
            if (result.IsValid)
            {
                var mapper = new MapperConfiguration(cfg => cfg.CreateMap<DirectoryDTO, CulturalViewModel>()).CreateMapper();
                return View(mapper.Map<DirectoryDTO, CulturalViewModel>(result.Result));
            }
            return _oops.OutPutError("Cultural", "Index", result.ErrorsList);
        }

        [HttpPost, ActionName("Update")]
        [Authorize(Roles = "Admin,Accountant,Deputy")]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> UpdateConfirmed([Bind("Id,Name")] CulturalViewModel vm)
        {
            if (ModelState.IsValid)
            {
